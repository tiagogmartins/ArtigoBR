...
# base
base <- ts(read_excel("base2000IAE.xlsx")[,-1], start = c(2000,1), freq = 12)
legenda <- data.frame(read_excel("base2000IAE.xlsx", sheet = 2)  )
...
#saveRDS(nowpastSM10,"nowpastSM10IAE.rds")
# saveRDS(nowpastSM11,"nowpastSM11.rds")


nowpastSM10 <- readRDS("nowpastSM10IAE.rds")
out <- nowpastSM10
out$nivel <- window(out$nivel, start = c(2015,1), freq = 4)
out$varQ <- window(out$varQ, start = c(2015,1), freq = 4)
out$varA <- window(out$varA, start = c(2015,1), freq = 4)
out$acum4 <- window(out$acum4, start = c(2015,1), freq = 4)

colMeans(nowpast.error(out = out, y = pib, yAS = pibAS, yVarQ = pibVarQ, yVarA = pibVarA)$trimestral)
# nivel     varQ     varA 
# 2.680150 1.136748 1.580387 

colMeans(nowpast.error(out = nowpastSM10, y = pib, yAS = pibAS, yVarQ = pibVarQ, yVarA = pibVarA)$trimestral)
mean(nowpast.error(out = nowpastSM10, y = pib, yAS = pibAS, yVarQ = pibVarQ, yVarA = pibVarA)$acumAno)
